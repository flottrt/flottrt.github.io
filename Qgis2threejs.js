...
(function () {
  ...
  app.showQueryResult = function (point, obj, hide_coords) {
    ...
    function wrapInLinkIfNeeded(value) {
      if (regex.test(value)) {
    ...
    if (e) {
      ...
      if (layer && layer.properties.propertyNames !== undefined) {
        ...
        for (var i = 0, l = layer.properties.propertyNames.length; i < l; i++) {
          ...
          var propertyValue = obj.userData.properties[i];
          row.innerHTML = "<td>" + layer.properties.propertyNames[i] + "</td>";
          var html = "<td>";
          if (Array.isArray(propertyValue)) {
            for (var j = 0; j < propertyValue.length; j++) {
              html += "<div>" + wrapInLinkIfNeeded(propertyValue[j]) + "</div>";
            }
          } else {
            html += wrapInLinkIfNeeded(propertyValue);
          }
          html += "</td>";
          console.log(html);
          row.innerHTML += html;
